...
export class CategoryProgram extends Program{
    ...
    protected async execute(cli:any,action?:string,arg1?:string,arg2?:string,arg3?:string): Promise<void> {
        if(action === "create"){
            await this.network(true).post("/v1/categories",{name:arg1,description:arg2});
        }else if(action === "modify"){
            await this.network(true).put(`/v1/categories/${arg1}`,{name:arg2,description:arg3});        
        }else if(action === "remove"){
            await this.network(true).delete(`/v1/categories/${arg1}`);
        }else{
            await this.printAllCategories();
        }
    }
    private async printAllCategories(){
        const {data : categories} = await this.network().get("/v1/categories");
        this.printLn("文章分类列表: \r\n");
